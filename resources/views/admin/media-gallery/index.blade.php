            ...
            @if ($media->isEmpty())
                <div class="card my-5">
                    <div class="card-body text-center py-5 text-muted">
                        ...
                        <p class="mb-0 mt-2">No media in gallery yet. <a
                                href="{{ route('admin.media-gallery.create') }}">Upload some</a>.</p>
                    </div>
                </div>
            @else
                    ...
                    @foreach ($media as $item)
                        <div class="media-item-card">
                            ...
                            <div class="media-thumb-box">
                                {{-- "Tag" for type --}}
                                @if ($item->isImage())
                                    <div class="position-absolute top-0 start-0 media-tag media-type-image">
                                        <i class="bi bi-image me-1"></i> Image
                                    </div>
                                @else
                                    <div class="position-absolute top-0 start-0 media-tag media-type-video">
                                ...
                                <div class="media-item-actions dropdown">
                                    <button class="dropdown-dot-btn" type="button"
                                        id="mediaDropdown{{ $item->id }}" data-bs-toggle="dropdown"
                                        aria-expanded="false">
                                        <i class="bi bi-three-dots-vertical"></i>
                                    </button>
                                    <ul class="dropdown-menu dropdown-menu-end"
                                        aria-labelledby="mediaDropdown{{ $item->id }}">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('admin.media-gallery.edit', $item) }}">
                                                <i class="bi bi-pencil-square me-1"></i> Edit
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('admin.media-gallery.attach-show', $item) }}">
                                                <i class="bi bi-link-45deg me-1"></i> Attach to Show
                                            </a>
                                        </li>
                                        <li>
                                            <form action="{{ route('admin.media-gallery.destroy', $item) }}"
                                ...
                                @if ($item->isImage())
                                    <img src="{{ $item->path }}" class="media-thumb-img"
                                        alt="{{ $item->title ?: $item->original_name }}">
                                @else
                                    @if ($item->thumbnail)
                                        <img src="{{ $item->thumbnail }}" class="media-thumb-img"
                                            alt="Video thumbnail">
                                    @else
                                        <div class="w-100 h-100 d-flex align-items-center justify-content-center"
                                            style="font-size: 2.3rem; color: #16a34a; opacity: 0.7">
