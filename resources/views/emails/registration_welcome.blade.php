...
@section('content')
    ...
    <h1 style="color: #5A10AC; font-size: 2.1rem; margin-bottom: 12px; margin-top: 0; font-family: 'Montserrat', Arial, sans-serif; letter-spacing: -1px;">Willkommen, {{ $user->name }}!</h1>
    ...
    <div style="background: #f0e8fc; border-radius: 12px; padding: 20px 24px 1px 24px; margin-bottom: 32px; box-shadow: 0 2px 10px 0 #ece9f5; max-width: 450px; margin-left: auto; margin-right: auto;">
    ...
    <div style="margin: 32px 0;">
        ...
        <div style="background: #f3eefb; border-radius: 8px; padding: 10px 12px;">
            <a href="{{ $user->referralLink() }}" style="color: #5A10AC; text-decoration: none; word-break: break-all;">{{ $user->referralLink() }}</a>
        </div>
    </div>

    <div style="margin: 32px 0;">
        <div style="font-weight: bold; color: #9E136D; font-size: 1.07rem; margin-bottom: 4px;">Dein Magic Link <span style="font-weight: normal; color: #6e6e6e;">(Ein-Klick-Login)</span></div>
        <p style="margin: 6px 0 10px 0; color: #555;">
            Mit diesem Link loggst du dich direkt ein und bist sofort live dabei:
        </p>
        <div style="background: #f3eefb; border-radius: 8px; padding: 10px 12px;">
            <a href="{{ $user->magicLink() }}" style="color: #9E136D; text-decoration: none; word-break: break-all;">{{ $user->magicLink() }}</a>
        </div>
    </div>
