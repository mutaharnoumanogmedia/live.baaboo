...
Route::middleware(['auth:web'])->group(function () {
    ...
    Route::post('live-show/{id}/send-message', [GamePlayController::class, 'postMessage'])->name('live-show.post-message')->middleware('throttle:40,1');
    Route::post('live-show/{id}/heart-reaction', [GamePlayController::class, 'heartReaction'])->name('live-show.heart-reaction')->middleware('throttle:5,1');
    // report user message
...
if (env('APP_ENV') == 'local') {
    ...
    Route::get('/test/live-show/{id}/add-users/{count}', function ($id, $count) {
        ...
        return response()->json(['message' => $count.' users added to live show'], 200);
    });
}
