<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Media that plays after all quiz questions in the show flow.
     * Kept separate from show-wide and question-level attachments.
     */
    public function up(): void
    {
        Schema::create('live_show_end_media', function (Blueprint $table) {
            $table->id();
            $table->foreignId('live_show_id')->constrained('live_shows')->cascadeOnDelete();
            $table->foreignId('gallery_media_id')->constrained('gallery_media')->cascadeOnDelete();
            $table->unsignedInteger('sort_order')->default(0);
            $table->boolean('media_played')->default(false);
            $table->timestamps();

            $table->unique(['live_show_id', 'gallery_media_id']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('live_show_end_media');
    }
};
